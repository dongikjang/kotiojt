...
## ---- page 077 ----------------------------------------------------------------------------------------------
par(mfrow=c(2,2), pch=16, cex.lab=1.5, cex=2, mar=c(5,5,4,2))
library(MASS)
attach(Animals)
plot(body, brain)
...
## ---- page 166 ----------------------------------------------------------------------------------------------
val <- "교통연구원"
if(.Platform$OS.type=="windows") val <- iconv("교통연구원", "CP949", "UTF-8")
result <- searchTwitter(val, since='2014-01-01', until='2014-05-11', n=1000, cainfo="cacert.pem")
result[1:3]
...
tweets <- userTimeline('rdatamining', n=400)
